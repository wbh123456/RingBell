...
def match_all(listeners, bell_ringers):
    for b in bell_ringers:
        ...
        matched_listener = b.find_listener(listeners)
        if matched_listener == -1:
            print("     Cannot find a Listener!")
        else:
            print("     Bell Ringer: ", b.name)
            print("     Listener:    ", matched_listener[0].name)
            print("     At Time:     ", matched_listener[1])
...
bellRingers = read_xls('../Data/real_examples.xls')
listerners = [Person(43773.1,"Tiger","1",[2,3]),Person(43773.1,"Bob","2",[5,10]),Person(43773.1,"Jenny","3",[18,19,20]),Person(43773.1,"Helen","4",[16])]
...
match_all(listerners, bellRingers)